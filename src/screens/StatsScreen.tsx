  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {(() => {
          // 1. Görev Başarı Oranı Hesaplama
          const { tasks, dailyStats } = useAppStore.getState(); // Ekstra veri çekimi
          const totalTasks = tasks.length;
          const completedTasks = tasks.filter(t => t.completed).length;
          const successRate = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

          // 2. Günlük Ortalama Odaklanma Süresi Hesaplama
          const totalWeekHours = dailyStats.reduce((sum, item) => sum + item.focusHours, 0);
          const activeDaysCount = dailyStats.filter(item => item.focusHours > 0).length;
          const avgHoursPerActiveDay = activeDaysCount > 0 ? (totalWeekHours / activeDaysCount).toFixed(1) : "0.0";
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.kpiItem}>
                <View style={styles.kpiLeft}>
                  <Text style={styles.kpiEmoji}>🎯</Text>
                  <View>
                    <Text style={styles.kpiTitle}>Günlük Ortalama</Text>
                    <Text style={styles.kpiDesc}>Aktif gün başına düşen odak saati</Text>
                  </View>
                </View>
                <Text style={styles.kpiValue}>{avgHoursPerActiveDay} sa</Text>
              </View>
              ...
              <View style={styles.kpiItem}>
                ...
                <Text style={styles.kpiValue}>{activeDaysCount}/7 Gün</Text>
              </View>
            </View>
          );
        })()}
