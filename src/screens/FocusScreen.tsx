...
export default function FocusScreen() {
  ...
  // Animasyon için referans değeri
  const fillAnimation = useRef(new Animated.Value(0)).current;

  // Kalan süre oranı (0.0 ile 1.0 arası)
  const progressRatio = timer.duration > 0 ? (timer.duration - timer.timeLeft) / timer.duration : 0;
  ...
  // Su dolma animasyonunu tetikleyen useEffect
  useEffect(() => {
    Animated.timing(fillAnimation, {
      toValue: progressRatio,
      duration: 1000, // Saniyelik akıcı geçiş
      useNativeDriver: true, // Performans için native grafik işlemcisini kullanır
    }).start();
  }, [progressRatio]);
  ...
  // Suyun Y eksenindeki hareketi (Çember boyutu olan 250'den 0'a doğru yükselir)
  const translateY = fillAnimation.interpolate({
    inputRange: [0, 1],
    outputRange: [250, 0], 
  });
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.timerCircleOuter}>
          ...
          {/* Metinler (Suyun üstünde kalması için absolute yapıldı) */}
...
const styles = StyleSheet.create({
  ...
  timerCircleOuter: {
    ...
    elevation: 8,
    marginBottom: 30,
    overflow: 'hidden', // Animasyonun çember dışına taşmasını engeller
    position: 'relative',
  },
  waterFill: {
    position: 'absolute',
    ...
    opacity: 0.25, // Metinlerin arkasında şık ve saydam durması için
  },
  timerCircleInner: {
    position: 'absolute', // Suyu arkaya atıp metinleri ortalamak için
    alignItems: 'center',
    justifyContent: 'center',
    width: '100%',
    height: '100%',
