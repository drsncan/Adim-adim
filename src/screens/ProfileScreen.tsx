  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {(() => {
          // Kullanıcının toplam odaklanma süresini saate çevir
          const totalHours = user.totalFocusTime / 3600;

          // Seviye hesaplama mantığı (Örnek: Her 10 saatte bir seviye atlar)
          const HOURS_PER_LEVEL = 10;
          
          // Mevcut seviye ve bir sonraki seviye için gereken saat
          const currentLevel = Math.floor(totalHours / HOURS_PER_LEVEL) + 1;
          const currentLevelHours = totalHours % HOURS_PER_LEVEL;
          
          // Yüzdelik ilerleme (Örn: 10 saatin 7'si dolduysa %70)
          const progressPercent = Math.min(Math.round((currentLevelHours / HOURS_PER_LEVEL) * 100), 100);

          // Seviyelere göre ünvanlar
          const getTitleForLevel = (level: number) => {
            if (level === 1) return "Odak Çaylağı 🌱";
            if (level === 2) return "Gelişen Odak 🌿";
            if (level === 3) return "Zaman Yöneticisi ⏱️";
            if (level === 4) return "Odaklanma Ustası 🎯";
            return "Odaklanma Gurusu 🌟";
          };

          return (
            <View style={styles.progressCard}>
              <Text style={styles.progressCardTitle}>Seviye {currentLevel} İlerlemesi</Text>
              
              <View style={styles.progressLabelRow}>
                <Text style={styles.progressLabel}>%{progressPercent} Tamamlandı</Text>
                <Text style={styles.progressDetails}>
                  {currentLevelHours.toFixed(1)} / {HOURS_PER_LEVEL} Saat
                </Text>
              </View>
              ...
              <Text style={styles.progressBadgeHint}>
                Bir sonraki seviye: "{getTitleForLevel(currentLevel + 1)}"
              </Text>
            </View>
          );
        })()}
